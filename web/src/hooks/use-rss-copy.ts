...
import { toast, getErrorMessage, getAppPath, shellClipboardWrite } from '@mochi/web'
...
export function useRssCopy(wiki: string) {
  ...
  const copy = async (mode: 'changes' | 'comments' | 'all', regenerate = false) => {
    try {
      ...
      const path = wiki === '*' ? `${getAppPath()}/-/rss` : `${getAppPath()}/${wiki}/-/rss`
      const url = new URL(`${path}?token=${token}`, window.location.href).href
      if (await shellClipboardWrite(url)) {
        toast.success(regenerate ? t`New RSS URL copied to clipboard` : t`RSS URL copied to clipboard`)
      }
    } catch (error) {
