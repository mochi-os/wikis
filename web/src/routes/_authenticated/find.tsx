...
function FindWikisPage() {
  ...
  const {
    ...
    queryFn: () =>
      requestHelpers.get<RecommendationsResponse>(
        `${getAppPath()}/${endpoints.wiki.recommendations}`
      ),
    retry: false,
  ...
  const subscribedWikiIds = useMemo(
    () =>
      new Set(
        (info?.wikis || []).flatMap((w) =>
          [w.id, w.fingerprint, w.source].filter((x): x is string => !!x)
        )
      ),
    [info?.wikis]
  )
  ...
  const handleSubscribe = useCallback(
    async (
      wikiId: string,
      entity?: { location?: string; fingerprint?: string; peer?: string }
    ) => {
      try {
